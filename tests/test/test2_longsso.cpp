...
TEST(ALog_LongSSO, string_constructor)
{
    ...
    EXPECT_STREQ(sso.getString(), "1");
}

TEST(ALog_LongSSO, format_fail)
{
    ALog::I::LongSSO<> sso("1");
    ...
    // It might also be necessary to adjust this test to the system it is running on.
        ...
        EXPECT_STREQ(sso.getString(), "12q"); // Msvc ignores the %
    #elif ALOG_COMPILER_APPLE_CLANG
        EXPECT_STREQ(sso.getString(), "12");  // Xcode ignores the type
    #else // Linux
        EXPECT_STREQ(sso.getString(), "12-- ALOG: Failed to format \"%q\" (Invalid argument)"); // Linux returns an error
    #endif
}
